...
export function BulkOpenEnvelopesPanel({
  ...
  const rows = envelopes.map((e) => {
    ...
    return { ...e, actualCents, diff };
  });

  const totalExpected = envelopes.reduce((s, e) => s + e.expectedAmountCents, 0);
  const totalActual = rows.reduce((s, r) => s + (r.actualCents ?? 0), 0);
  const allFilled = rows.every((r) => r.actualCents !== null);

  const batchPayload = rows
    ...
    <div className="mt-4 rounded-lg border bg-white shadow-sm">
        ...
        <table className="w-full text-sm">
                ...
                <td className="px-3 py-2">
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <input
                      ...
                      className="w-28 rounded border px-2 py-1 text-right text-sm font-semibold"
                    />
                  </div>
                </td>
          ...
          <tfoot className="bg-neutral-50">
            <tr className="border-t">
              <td colSpan={4} className="px-3 py-2 text-xs font-semibold text-neutral-600">
                Total ({envelopes.length} sobres)
              </td>
      ...
      <div className="flex items-center justify-between border-t px-4 py-3">
        ...
        >
          ...
          >
            {submitting ? "Guardando…" : `Confirmar ${envelopes.length} sobres`}
          </button>
        </form>
      </div>
    </div>
